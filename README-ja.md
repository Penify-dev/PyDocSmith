...
- **カスタムレンダリングスタイル:** コンパクトまたは詳細なスタイルで docstring のレンダリングをカスタマイズし、生成された docstring テキストのインデントを指定できます。

## docstring_parser に対して変更された点

1. docstring スタイル検出のためのより良いヒューリスティック
2. Google Docstring が Notes、Examples を収容するように変更
3. GPT や Mistral などの LLM から生成された場合、GoogleDoc string が適切なインデントを持たないことがある。PyDocSmith はこれらの不良 docstring を修正できます。
