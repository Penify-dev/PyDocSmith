...
- **Docstring スタイル検出：** シンプルなヒューリスティクスを使用して docstrings のスタイル（例: reST、Google、NumPydoc、Epydoc）を自動的に検出します。
- **Docstring 解析：** docstrings を構造化表現に変換し、ドキュメントの分析と操作を容易にします。
- **Docstring 作成：** 構造化された docstrings をテキストに戻してレンダリングし、自動化された docstring 生成と変更を可能にします。
...
- **オブジェクトからの解析：** PyDocSmith は、クラスやモジュールを含む Python オブジェクトから直接 docstrings を解析でき、構造化表現に属性 docstrings を組み込みます。
...
1. Docstring スタイル検出のためのより良いヒューリスティクス
...
私はこれをユースケースに基づいて更新しました - https://www.penify.dev
