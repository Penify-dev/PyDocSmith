...
PyDocSmith は、さまざまなスタイルでドックストリングを解析、検出、作成するために設計された多用途の Python パッケージです。reStructuredText (reST)、Google、NumPydoc、Epydoc などの複数のドックストリング規約をサポートし、Python 開発者のドキュメント実践に柔軟性を提供します。
...
- **ドックストリングの作成：** 構造化ドックストリングをテキストに戻してレンダリングし、自動ドックストリング生成と変更を可能にします。
...
2. Google ドックストリングは Notes、Examples を収容するために変更されました
3. 時々 GoogleDoc 文字列は、特に GPT や Mistral などの LLMs から生成された場合、正しいインデントを持っていません。PyDocSmith はこれらの悪いドックストリングを修正できます。
4. GoogleDocstring の異なるスタイルを収容するために追加のテストケースが追加されました
