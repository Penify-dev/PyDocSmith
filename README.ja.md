...
- **カスタムレンダリングスタイル:** コンパクトまたは詳細なスタイルで docstring のレンダリングをカスタマイズし、生成された docstring テキストにカスタムのインデントを指定できます。

## docstring_parser に対して変更された点

1. Docstring スタイルを検出するためのより優れたヒューリスティック
2. Google Docstring が Notes、Examples を収容するように変更されました
3. GPT や Mistral などの LLM から生成された場合、GoogleDoc string が適切なインデントを持たないことがあります。PyDocSmith はそれらの不良 docstring を修正できます。
