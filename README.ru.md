...
- **Докстринги атрибутов:** Разбирать докстринги атрибутов, определенные на уровне класса и модуля, улучшая документацию свойств класса и переменных уровня модуля.
...
- **Пользовательские стили рендеринга:** Настраивать рендеринг докстрингов с компактными или детальными стилями, и указывать пользовательскую отступ для сгенерированного текста докстринга.

## Вещи, которые были изменены по сравнению с docstring_parser

1. Лучшие эвристики для обнаружения стиля докстринга
2. Докстринг Google был изменен для размещения Заметок, Примеров
3. Иногда докстринг Google не имеет правильного отступа, особенно когда генерируется из LLM вроде GPT или Mistral. PyDocSmith может исправить эти плохие докстринги.
